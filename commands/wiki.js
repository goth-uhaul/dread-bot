...
const parsePage = (content, title, path) => {
  ...
  if (
    content.match("<h1>") ||
    content.match("# Tabs {.tabset}") ||
    content.split(/^#[^#]/m).find((x) => x.length > 6000)
  )
    return [
      {
        header: title,
        body:
          "This page is too complex to display on Discord! Please view it [here](https://" +
          wikiDomain +
          "/en/" +
          path +
          ") instead.",
  ...
  content = content
    .replace(/\n{\.(?:(?:links)|(?:grid))-list}/g, "")
    .replace(/\[(.+?)\]\((.+?)\)/g, (match, x, y) =>
      y.startsWith("/") ? "[" + x + "](https://" + wikiDomain + y + ")" : match
    )
    .replace(
      /> (.+)\n{\.(.+)}/g,
      (match, x, y) => "> " + blockquoteEmojis[y] + " " + x
    );
  ...
  sections = sections.map((x) => {
    ...
    section.subsections.forEach((y) => {
      ...
      subsection.body = split
        .map((z) =>
          z.replace(/\|(.+)\|/, (match, rowContent) =>
            rowContent.split("|").join(" - ")
          )
        )
        .join("\n")
        .trim();
...
const sectionToEmbed = (section) => {
  ...
  if (section.subsections)
    section.subsections.map((x) =>
      embed.addFields({ name: x.header, value: x.body ? x.body : "\u200b" })
    );
...
const singleQuery = (id, elements) =>
  "{pages{single(id:" + id + "){" + elements.join(" ") + "}}}";
const listQuery = (elements) => "{pages{list{" + elements.join(" ") + "}}}";

// Fetch list of pages
const listPages = (elements) =>
  new Promise((resolve, reject) =>
    axios
      .get(
        "https://" + graphQlDomain + "/graphql?query=" + listQuery(elements),
      ...
      let data = await axios
        .get(
          "https://" +
            graphQlDomain +
            "/graphql?query=" +
            singleQuery(id, ["title", "content", "path"]),
          { headers: { Authorization: "Bearer " + wikiToken } }
        )
        .catch((e) => reject(e));
      if (data.data.errors)
...
const fetchPageIndex = async () =>
  (pagesIndex = await listPages(["id", "title"]).catch(console.error));
...
const userQuery = (id) =>
  '{users{singleByProviderId(providerId:"' +
  id +
  '"){id,providerId,name,providerName}}}';
const fetchUser = (id) =>
  new Promise((resolve) =>
    axios
      .get("https://" + graphQlDomain + "/graphql?query=" + userQuery(id), {
        headers: { Authorization: "Bearer " + wikiToken },
      })
      .then((res) => resolve(res.data.data.users.singleByProviderId))
  );
            ...
            "link",
            {
              ...
              execute(interaction) {
                return new Promise(async (resolve, reject) => {
                  ...
                  let page = pagesIndex.find(
                    (p) =>
                      p.title.toLowerCase() ===
                      interaction.options.getString("page").toLowerCase()
                  );
                  if (!page)
                    return interaction
                      .reply({ content: "No page found!", ephemeral: true })
                      .then(resolve())
                      .catch((e) => reject(e));
                  ...
                  // Send reply
                  interaction
                    .reply("https://" + wikiDomain + "/en/" + page.path)
                    .then(resolve())
                    .catch((e) => reject(e));
                });
              },
              autocomplete(interaction) {
                return new Promise((resolve, reject) => {
                  ...
                  let pages = pagesIndex.filter((p) =>
                    p.title
                      .toLowerCase()
                      .startsWith(interaction.options.getFocused())
                  );
                  ...
                  interaction
                    .respond(
                      pages.map((p) => ({ name: p.title, value: p.title }))
                    )
                    .then(resolve())
                    .catch((e) => reject(e));
                });
                    ...
                    toSend.components = [
                      new ActionRowBuilder().addComponents(
                        client.buttons
                          ...
                          .button(pageId)
                          .setDisabled(true),
                        client.buttons.get("pageForward").button(pageId)
                      ),
                    ];
        ...
        execute(interaction) {
          return new Promise(async (resolve, reject) => {
            ...
            if (interaction.member.roles.cache.has(contributorRole))
              return interaction
                .reply({
                  content: "You already have the contributor role!",
                  ephemeral: true,
                })
                .then(resolve())
                .catch((e) => reject(e));
            ...
            if (!user)
              return interaction
                .reply({ content: "No user found!", ephemeral: true })
                .then(resolve())
                .catch((e) => reject(e));
            ...
            await interaction.member.roles
              ...
              .catch((e) => reject(e));
            interaction
              .reply("User found! You should now have the contributor role.")
              .then(resolve())
              .catch((e) => reject(e));
          });
        },
