...
for (const file of commandFiles) {
  ...
  if (command.subcommandGroups) {
    command.subcommandGroups.forEach((v, k) => {
      v.subcommands.forEach((vv, kk) =>
        enabledComponents.includes(vv.component)
          ? v.data.addSubcommand(vv.data)
          : v.data.delete(kk)
      );
      enabledComponents.includes(v.component)
        ? command.data.addSubcommandGroup(v.data)
        : command.subcommandGroups.delete(k);
    });
  }
  if (command.subcommands)
    command.subcommands.forEach((v, k) =>
      enabledComponents.includes(v.component)
        ? command.data.addSubcommand(v.data)
        : command.subcommands.delete(k)
    );
  if (enabledComponents.includes(command.component))
...
for (const file of contextMenus) {
  ...
  if (enabledComponents.includes(contextMenu.component))
...
for (const file of modalFiles) {
  ...
  if (enabledComponents.includes(modal.component))
    client.modals.set(file.slice(0, -3), modal);
}
...
for (const file of buttonFiles) {
  ...
  if (enabledComponents.includes(button.component))
...
for (const file of selectMenuFiles) {
  ...
  if (enabledComponents.includes(selectMenu.component))
...
registerCommands(
  client.commands
    .map((c) => c.data)
    .concat(client.contextMenus.map((c) => c.data))
);
...
client.on(Events.InteractionCreate, (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (
      ...
      command = command.subcommandGroups
        .get(interaction.options.getSubcommandGroup())
        .subcommands.get(interaction.options.getSubcommand());
    else if (command.subcommands && interaction.options.getSubcommand(false))
      command = command.subcommands.get(interaction.options.getSubcommand());
  ...
  // Context menus
    ...
    if (contextMenu.ownerOnly && !owners.includes(interaction.user.id))
      return interaction.reply({
        content: "Only the bot owners can use this command!",
        ephemeral: true,
      });
  ...
  // Modal submits
    ...
    const modal = client.modals.get(
      pos === -1 ? interaction.customId : interaction.customId.slice(0, pos)
    );
...
if (enabledComponents.includes("streams")) {
  const objectsArrayEquals = (arr1, arr2) => {
    if (arr1.length !== arr2.length) return false;
      ...
      return arr1.every(
        (x, i) => JSON.stringify(x) === JSON.stringify(arr2[i])
      );
  };

  client.on(Events.PresenceUpdate, async (oldPresence, newPresence) => {
    if (newPresence.guild.id !== dreadServer) return;
    const streams = newPresence.activities.filter(
      (activity) =>
        activity.type === ActivityType.Streaming &&
        activity.state === "Metroid Dread"
    );
    if (streams.length > 0) {
      if (
        !oldPresence ||
        ...
        const user = await StreamBlacklist.findOne({
          where: { userId: newPresence.user.id },
        });
        if (!user) {
          streams.forEach((stream) => {
            client.channels
              .fetch(streamsChannel)
              .then((c) =>
                c.send({ embeds: [streamEmbed(stream, newPresence.user)] })
              );
          });
